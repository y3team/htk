...
def get_follower_data():
    try:
        ...
        if 'data' in data and 'follower' in data['data']:
            ...
            # 检查 webview.windows 是否有内容
            if len(webview.windows) > 0:
                webview.windows[0].evaluate_js(f"window.updateFollowerData({json.dumps(data)})")
            else:
                print("Webview 窗口未初始化或不存在")
                error_message = {"错误代码": -1, "信息": "Webview 窗口未初始化或不存在"}
                if len(webview.windows) > 0:
...
class Api:
    ...
    def __init__(self, base_path):
        ...
        # 将独立的 get_follower_data 方法绑定到类中
    def getFollowerData(self):
        try:
            ...
            bilibili_api_response = requests.get(url, headers=headers)
            bilibili_api_response.raise_for_status()
            data = bilibili_api_response.json()

            if 'data' in data and 'follower' in data['data']:
                return {'code': 0, 'data': {'follower': data['data']['follower']}}
            else:
                return {'code': -1, 'message': '数据结构不符合预期'}
        except requests.RequestException as e:
